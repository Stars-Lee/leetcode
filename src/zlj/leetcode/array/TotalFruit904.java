...
public class TotalFruit904 {
    public int totalFruit(int[] fruits) {
        ...
        //存储从第i棵开始每种类型的果树数量
        Map<Integer, Integer> typeMap = new HashMap<>(2);
        int maxL = -1;
        int sum = 0;
        int start;
        for (int i = 0; i < fruits.length; i++){
            if (typeMap.containsKey(fruits[i])){
                ...
                start = i - 1;
                int preCount = 0;
                while (fruits[start] == fruits[i - 1]){
                    start--;
                    preCount++;
                }
                typeMap.clear();
                typeMap.put(fruits[i - 1], preCount);
                typeMap.put(fruits[i], 1);
                sum = preCount + 1;
            }
        }
    ...
    public void test(){
        ...
        System.out.println(totalFruit(nums2));
        System.out.println(totalFruit(nums3));
